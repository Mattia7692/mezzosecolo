export function buildInviteEmail({ name, token, address, mapsEmbedUrl, customText }) {
  const baseUrl = process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'
  const rsvpUrl = `${baseUrl}/rsvp/${token}`

  const mapsSection = mapsEmbedUrl
    ? `
      <div style="margin: 24px 0; border-radius: 8px; overflow: hidden; border: 1px solid #e8e0d0;">
        <iframe
          src="${mapsEmbedUrl}"
          width="100%"
          height="280"
        ...
        <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px; width:100%; background-color:#faf8f3; border-radius:12px; overflow:hidden; box-shadow: 0 20px 60px rgba(0,0,0,0.5);">
          ...
          <tr>
            <td style="padding: 40px 48px;">
              ...
              <p style="margin:0 0 24px; font-size:20px; color:#2d2d2d;">Caro/Cara <strong>${name}</strong>,</p>
              ...
              <table width="100%" cellpadding="0" cellspacing="0">
                <tr>
                  <td style="padding:16px 0; border-bottom:1px solid #f0ece4;">
                    ...
                    <p style="margin:0; font-size:18px; color:#2d2d2d; font-weight:bold;">Giovedì 18 Giugno 2026</p>
                    <p style="margin:4px 0 0; font-size:15px; color:#6a6a6a;">dalle ore 19:00</p>
                  </td>
                </tr>
                <tr>
                  <td style="padding:16px 0;">
                    ...
                    <p style="margin:0; font-size:18px; color:#2d2d2d;">${address || 'Luogo da definire'}</p>
                  </td>
                </tr>
              </table>
